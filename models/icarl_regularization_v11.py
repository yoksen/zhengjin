...
class icarl_regularization_v11(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self._total_classes != sum(data_manager._increments):
            ...
            self._eval_classifier(data_manager=data_manager)

        if len(self._multiple_gpus) > 1:
            self._network = self._network.module
            self._generator = self._generator.module
    
    def _eval_classifier(self, data_manager):
        train_dataset = data_manager.get_dataset([], source='train', mode='train', appendent=(self._data_memory, self._targets_memory))
        ...
        info = 'Classifier on original training data Train_accy {:.2f}'.format(train_acc)
        logging.info(info)

        train_dataset = data_manager.get_dataset([], source='train', mode='train', appendent=(self._inverse_data_memory, self._inverse_targets_memory))
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    ...
    def _eval_generator(self, data_manager):
        ...
        for _, (_, inputs, targets) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ret_dict, targets = self._generator(inputs, targets)
                outputs = ret_dict['logits']
