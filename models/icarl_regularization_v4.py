...
class icarl_regularization_v4(BaseLearner):
    def __init__(self, args):
        print('create icarl_regularization_v4!!')
        super().__init__(args)
    ...
    def save_model(self):
        if len(self._multiple_gpus) > 1:
            torch.save(self._network.module.state_dict(), "./saved_model/icarl_regularization_v4_duplex_{}_{}.pt".format(duplex, self._cur_task))
        else:
            torch.save(self._network.state_dict(), "./saved_model/icarl_regularization_v4_duplex_{}_{}.pt".format(duplex, self._cur_task))
    ...
    def incremental_train(self, data_manager):
        ...
        if duplex:
            self._train_inverse_classifier(self.train_new_loader, self.test_new_loader)
    ...
    def _train_inverse_classifier(self, train_loader, test_loader):
        self._network.to(self._device)
        if self._old_network is not None:
            self._old_network.to(self._device)
        ...
        if self._cur_task == 0:
            ...
            optimizer = optim.SGD(self._network.parameters(), lr=lrate, momentum=0.9, weight_decay=weight_decay)  # 1e-5
            scheduler = optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=milestones, gamma=lrate_decay)
        self._update_representation_inverse(train_loader, test_loader, optimizer, scheduler)
    ...
    def _update_representation_inverse(self, train_loader, test_loader, optimizer, scheduler):
        if self._cur_task == 0:
            epochs_num = epochs_init
        else:
            epochs_num = epochs
    ...
    def _reduce_exemplar(self, data_manager, m):
        ...
        self._inverse_class_means = np.zeros((self._total_classes, self.feature_dim))
        ...
        for class_idx in range(self._known_classes):
            ...
            self._data_memory = np.concatenate((self._data_memory, inverse_old_class_images_)) if len(self._data_memory) != 0 else inverse_old_class_images_
            self._targets_memory = np.concatenate((self._targets_memory, dt)) if len(self._targets_memory) != 0 else dt

            # Inverse exemplar mean
            idx_dataset = data_manager.get_dataset([], source='train', mode='test', appendent=(inverse_old_class_images_, dt))
            ...
            self._inverse_class_means[class_idx, :] = mean
    ...
    def _construct_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            self._data_memory = np.concatenate((self._data_memory, inverse_old_class_images_)) if len(self._data_memory) != 0 \
                else inverse_old_class_images_
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            _inverse_class_means[class_idx, :] = mean
