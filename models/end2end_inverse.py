...
hyperparameters = ["epochs_init", "lrate_init", "milestones_init", "lrate_decay_init","weight_decay_init",\
                   "epochs","lrate", "milestones", "epochs_finetune", "lrate_finetune", "milestones_finetune",\
                   "lrate_decay","batch_size","T","weight_decay","num_workers"]
...
class End2End(BaseLearner):
    ...
    def _train(self, data_manager, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            optimizer = optim.SGD(self._network.parameters(), lr=lrate_init, momentum=0.9, weight_decay=weight_decay_init)
            scheduler = optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=milestones_init,
                                                       gamma=lrate_decay_init)
