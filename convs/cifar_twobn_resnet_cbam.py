...
class CifarResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, norm_layer=None):
        ...
        self.bn_1 = norm_layer(64)


        self.inplanes = 64
        self.stage_1 = self._make_layer(block, 64, layers[0], 1)
...
class CifarAdvResNet(CifarResNet):
    ...
    def __init__(self, block, layers, num_classes=100,
        ...
        super().__init__(block, layers, num_classes=num_classes, norm_layer=norm_layer)
...
def resnet18_cbam(mix = False, num_classes = 100):
    """Constructs a ResNet-18 model for CIFAR-10."""
    if mix:
        attacker = PGDAttacker(num_iter=1, epsilon=1, step_size=1)
        model = CifarAdvResNet(ResNetBasicblock, layers=[2, 2, 2, 2], num_classes=num_classes, norm_layer=MixBatchNorm2d , attacker=attacker)
        model.set_mixbn(True)
    else:
        model = CifarAdvResNet(ResNetBasicblock, layers=[2, 2, 2, 2], num_classes=num_classes)
    return model